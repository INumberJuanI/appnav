...
const NavItem = ({ text, href, icon, activeElement, changeActiveElement }: Props) => {
    return (
        ...
        >
            <Link href={href} className='relative flex justify-center items-center flex-col	w-full text-center font-normal'>
                <span className={`icon | relative block leading-[75px] text-[1.5em] text-center transition-duration-[.5s] 
                    ${activeElement === text? "translate-y-[-32px]": ""}`}
                >
                    {icon}
                </span>
                <span className={`text | absolute font-normal text-[.75em] tracking-wider transition-duration-[.5s] 
                ...
                <span className={`circle | absolute block w-[50px] h-50px bg-transparent rounded-full border-[1px] border-solid border-white 
                    ${activeElement === text? "transition-duration-[.5s] delay-[.5s] translate-y-[-37px] scale-100" : "translate-y-[-37px] scale-0"}`}
                ></span>
            </Link>
        </li>
    )
